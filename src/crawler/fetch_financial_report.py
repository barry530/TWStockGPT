...
def fetch_monthly_revenue(year: int, month: int):
    ...
    if len(data) == 0:
        ...
        return None
    data = pd.concat(data)
    save_name = f'{year}年{str(month).zfill(2)}月營收.csv'
    data.to_csv(TEMP_PATH / save_name, index=False)
    return None

def fetch_seasonal_report(year: int, season: int, market_type, report_type):
